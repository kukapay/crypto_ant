...
class UI:
    ...
    def print_user_query(self, query: str):
        ...
        rich.print(Panel(query, title="QUESTION", border_style="bright_blue"))
    ...
    def print_task_list(self, tasks):
        ...
        plan = "\n".join([ f"{task.get('id')}. " + task.get('description')  for task in tasks ])
        rich.print(Panel(Markdown(plan), title="Planned Tasks", border_style="bright_blue"))    
        return
    ...
    def stream_answer(self, text_chunks: Iterator[str]) -> str:
        ...
        """
        answer = ""
        with Live(Panel("", title="ANSWER", border_style="bright_blue")) as live:
            for chunk in text_chunks:
                answer += chunk
                live.update(Panel(Markdown(answer), title="ANSWER", border_style="bright_blue"))              
        return answer
